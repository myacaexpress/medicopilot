...
import { Heart, Coins, Sprout, Shield, CheckCircle, Circle, AlertTriangle, Send, ChevronRight, Eye, EyeOff, Maximize2, Minimize2, Phone, User, Bot, Mic, MicOff, Monitor, GripVertical, Zap, Volume2, GripHorizontal } from "lucide-react";
...
function useResizable(iw, ih, minW = 320, minH = 400, maxW = 700, maxH = 900) {
  ...
  useEffect(() => {
    const mv = (e) => {
      ...
      const dw = e.clientX - startMouse.current.x;
      const dh = e.clientY - startMouse.current.y;
      setSize({
        w: Math.min(maxW, Math.max(minW, startSize.current.w + dw)),
        h: Math.min(maxH, Math.max(minH, startSize.current.h + dh)),
      });
    };
...
function MediCopilotOverlay({ mode, setMode, opacity }) {
  ...
  const [visibleTranscript, setVisibleTranscript] = useState(6); // how many transcript lines visible
  const [shownResponses, setShownResponses] = useState(1); // how many AI responses shown
  const [tick, setTick] = useState(0);
  const [textScale, setTextScale] = useState(1); // 0.85 / 1 / 1.15 / 1.3
  const [splitPct, setSplitPct] = useState(35); // % of content area given to transcript
  ...
  // Divider drag between transcript & AI responses
  ...
  useEffect(() => {
    const mv = (e) => {
      ...
      setSplitPct(Math.min(75, Math.max(15, pct)));
    };
      ...
      <div onMouseDown={collapsed.onMouseDown} style={{
        ...
        <div style={{ padding: "0 12px 6px" }}>
          <div style={{
            ...
            background: "rgba(255,255,255,0.03)", borderRadius: 6, borderLeft: "2px solid rgba(52,199,123,0.4)",
          }}>
        ...
        <div style={{ padding: "0 12px 8px" }}>
          <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: "8px 10px", borderLeft: `2px solid ${T.teal}` }}>
            <div style={{ fontFamily: T.body, fontSize: 12, color: `rgba(255,255,255,${Math.min(opacity+0.1,1)})`, lineHeight: 1.5 }}>
              {latestResp.response}
            </div>
            {latestResp.plans && (
              <div style={{ fontFamily: T.mono, fontSize: 10, color: "rgba(255,255,255,0.4)", marginTop: 4 }}>
                {latestResp.plans.map(p => p.name).join(" · ")}
              </div>
            )}
            {latestResp.detail && (
              <div style={{ fontFamily: T.body, fontSize: 11, color: "rgba(255,255,255,0.5)", marginTop: 4 }}>{latestResp.detail}</div>
            )}
          </div>
        </div>
        ...
        <div data-no-drag="true" style={{ padding: "0 12px 10px", display: "flex", gap: 6 }}>
          <div style={{
            ...
            borderRadius: 10, padding: "2px 2px 2px 12px",
          }}>
            ...
            />
            <button onClick={handleAskAI} style={{ background: T.teal, border: "none", borderRadius: 8, padding: "8px 10px", cursor: "pointer" }}>
              <Send size={13} color={T.white} />
            </button>
          </div>
          {/* Big Ask AI hotkey button */}
  ...
  return (
    <div onMouseDown={expanded.onMouseDown} style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        {/* Text size toggle */}
        <button data-no-drag="true" onClick={cycleTextSize} title={`Text: ${Math.round(textScale * 100)}%`} style={{
          display: "flex", alignItems: "center", gap: 2, background: "rgba(255,255,255,0.04)",
        ...
        <button data-no-drag="true" onClick={() => setMode("collapsed")} style={{ background: "none", border: "none", cursor: "pointer", padding: 2 }}>
          <Minimize2 size={14} color="rgba(255,255,255,0.5)" />
        </button>
        <button data-no-drag="true" onClick={() => setMode("hidden")} style={{ background: "none", border: "none", cursor: "pointer", padding: 2 }}>
          <EyeOff size={14} color="rgba(255,255,255,0.5)" />
        </button>
      ...
      <div data-no-drag="true" style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
        ...
        <button onClick={() => setAudioOn(!audioOn)} style={{
          ...
          {audioOn ? <Volume2 size={11} color="#34C77B" /> : <MicOff size={11} color="rgba(255,255,255,0.3)" />}
          <AudioWave active={audioOn} color="#34C77B" bars={4} />
          <span style={{ fontFamily: T.display, fontSize: 9, fontWeight: 600, color: audioOn ? "#34C77B" : "rgba(255,255,255,0.3)" }}>
        ...
        <button onClick={() => setScreenOn(!screenOn)} style={{
          ...
          <Monitor size={11} color={screenOn ? T.teal : "rgba(255,255,255,0.3)"} />
          <span style={{ fontFamily: T.display, fontSize: 9, fontWeight: 600, color: screenOn ? T.teal : "rgba(255,255,255,0.3)" }}>
      ...
      {/* Split container: Transcript | Divider | AI Responses */}
      <div ref={splitContainerRef} data-no-drag="true" style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", cursor: "default", minHeight: 0 }}>
        ...
        {/* Live Audio Transcript — top pane */}
          ...
          <div style={{ padding: "4px 14px 8px" }}>
            {transcriptLines.slice(0, visibleTranscript).map((line, i) => (
              <div key={i} style={{ display: "flex", gap: 8, padding: "3px 0", alignItems: "flex-start" }}>
                ...
                <span style={{
                  fontFamily: T.display, fontSize: 9, fontWeight: 600, minWidth: 40,
                  color: line.speaker === "client" ? T.coral : "rgba(255,255,255,0.3)",
                }}>
                  {line.speaker === "client" ? "Client" : "Agent"}
                </span>
                <span style={{
                  fontFamily: T.body, fontSize: scaledFont(12), lineHeight: 1.45,
                  color: line.isQuestion
                  ...
                  {line.isQuestion && (
                    <span style={{
                      marginLeft: 6, fontFamily: T.display, fontSize: 8, fontWeight: 700,
                      color: T.teal, background: "rgba(0,123,127,0.15)", padding: "1px 5px",
        ...
        {aiResponses.slice(0, shownResponses).map((resp, i) => (
          <div key={i} style={{ marginBottom: 14 }}>
            ...
            <div style={{
              ...
              {/* Trifecta bridge */}
              {resp.trifecta && <div style={{
                display: "flex", gap: 6, marginTop: 8, padding: "6px 8px",
                background: "rgba(0,123,127,0.06)", border: "1px solid rgba(0,123,127,0.15)", borderRadius: 6,
              }}>
              ...
              {/* Compliance */}
              {resp.compliance && <div style={{
                display: "flex", gap: 6, marginTop: 8, padding: "6px 8px",
                background: "rgba(245,166,35,0.06)", border: "1px solid rgba(245,166,35,0.12)", borderRadius: 6,
              }}>
              ...
              {/* Script */}
              {resp.script && <div style={{
                marginTop: 8, padding: "8px 10px",
                background: "rgba(0,123,127,0.06)", borderLeft: `2px solid ${T.teal}`, borderRadius: "0 6px 6px 0",
              }}>
      ...
      <div data-no-drag="true" onMouseDown={onResizeStart} style={{
  ...
  return (
    <div style={{ width: "100%", height: "100vh", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        padding: "4px 12px", background: "rgba(0,0,0,0.5)", borderRadius: 8,
        fontFamily: T.display, fontSize: 11, color: "rgba(255,255,255,0.5)",
      }}>
      ...
      {/* Demo controls */}
      <div style={{
        position: "absolute", bottom: 65, left: 16,
        display: "flex", gap: 6, padding: "8px 12px", alignItems: "center",
        background: "rgba(0,0,0,0.6)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)",
      }}>
